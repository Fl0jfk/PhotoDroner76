      ...
      <>
        {dataToDisplay && 
          <section className="flex flex-col gap-4 w-[35%] sm:w-full sm:items-center sm:mt-[5vh] sm:gap-3 z-2">
              ...
              <Link className="w-fit rounded-full border-[1px] border-[#ffffff8f] px-6 py-3 text-[15px] text-white font-thin transition duration-300 ease-in-out hover:bg-white hover:text-black active:bg-white active:text-black" href={`/projects/${encodeURIComponent(dataToDisplay.id)}`}>DECOUVREZ CE PROJET</Link>
          </section> 
        }
      </>
    );
